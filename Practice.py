...
.appt-avatar {
    ...
    border-radius: 50%;
    background: linear-gradient(135deg, #4EADA8, #357874);
...
/* SUGGEST CARD */
.sug-card {
    background: linear-gradient(135deg, #FFFAF6 0%, white 100%);
    border: 1.5px solid var(--peach);
    border-radius: 16px;
    padding: 1rem 1.2rem;
    margin-bottom: 0.7rem;
}
.sug-rank { font-family:'Nunito',sans-serif; font-weight:900; font-size:1.5rem; color:var(--rose); }
...
/* SUMMARY BOX */
.summary-box {
    background: linear-gradient(135deg, var(--teal-lt), white);
...
/* PROFILE CARD */
.profile-card {
    background: linear-gradient(135deg, #FEF3EC, white);
    border-radius: 20px;
    padding: 1.5rem;
    border: 1.5px solid var(--peach);
    text-align: center;
    box-shadow: 0 4px 16px rgba(245,166,125,0.15);
}
.profile-avatar {
...
with tab2:
    ...
    def warm_chart(df, col, color, title, ymin=None, ymax=None, hlines=[]):
        ...
        fig.update_layout(
            ...
            height=220, plot_bgcolor='#FFFAF6', paper_bgcolor='white',
